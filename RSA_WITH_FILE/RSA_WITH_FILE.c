...
void file_ecryption(char* nameOfFile,public_key publicKey,private_key privateKey){
    char  msg[250];
    char firstmsg[250];
    mpz_t int_message;
  ...
  if ((fptr = fopen(nameOfFile,"r")) == NULL){
       printf("Error! opening file");
       exit(1);
   }


    fgets( msg, 250, (FILE*)fptr);
    printf("The original message in file: %s\n",  msg );
    ...
    printf("\nThe encrypted message from file: %s\n", mpz_get_str(NULL, 0, encrypted));
    ...
    fprintf(fp, mpz_get_str(NULL, 0, encrypted));
...
int main() {
    ...
    printf("----------------------------------------------------\n");
    file_ecryption("testing_file.txt",publicKey,privateKey);
    printf("----------------------------------------------------\n");
